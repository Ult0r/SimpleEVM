...
class MerklePatriciaTrieTest {
  ...
  def void testEmptyTrie() {
    Assert.assertEquals(new MerklePatriciaTrie().root.hash.elements.toHex, "0x56E81F171BCC55A6FF8345E692C0F86E5B48E01B996CADC001622FB5E363B421")
  }
  ...
  def void testThreeEntries() {
    ...
    trie.putElement(new NibbleList("doe".bytes.map[new UnsignedByte(it)].toNibbles), "reindeer".bytes.map[new UnsignedByte(it)])
    trie.putElement(new NibbleList("dog".bytes.map[new UnsignedByte(it)].toNibbles), "puppy".bytes.map[new UnsignedByte(it)])
    trie.putElement(new NibbleList("dogglesworth".bytes.map[new UnsignedByte(it)].toNibbles), "cat".bytes.map[new UnsignedByte(it)])
  ...
  def private void addAccount(MerklePatriciaTrie trie, EVMWord address, EVMWord balance, EVMWord storageRoot, EVMWord codeHash) {
