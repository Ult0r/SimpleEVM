...
final class LoggerController {
  ...
  def static Logger createLogger(String name) {
    ...
    loggerControllerLogger.logp(Level.INFO, LoggerController.extractClassName, "createLogger", "logger created: " + name)
    logger
  }
  ...
  def static void removeLogger() {
    if (!loggerStack.empty()) {
      loggerControllerLogger.logp(Level.INFO, LoggerController.extractClassName, "removeLogger", "logger removed: " + getLogger.name)
      for (h: loggerStack.pop.handlers) {
        h.close
      }
    } else {
      loggerControllerLogger.logp(Level.WARNING, LoggerController.extractClassName, "removeLogger", "accessed empty Stack")
    }
  }
